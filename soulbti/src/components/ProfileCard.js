...
const ProfileCard = React.forwardRef(({ 
  ...
  const getTopDimensions = () => {
    return Object.keys(dimensions)
      ...
      .map(key => {
        ...
        switch(key) {
          case 'energy': code = score > 0 ? 'E' : 'I'; name = score > 0 ? '外向' : '内敛'; break;
          case 'information': code = score > 0 ? 'N' : 'S'; name = score > 0 ? '直觉' : '实感'; break;
          case 'decision': code = score > 0 ? 'F' : 'T'; name = score > 0 ? '感性' : '理性'; break;
          case 'lifestyle': code = score > 0 ? 'P' : 'J'; name = score > 0 ? '随性' : '计划'; break;
          case 'selfworth': code = score > 0 ? 'H' : 'L'; name = score > 0 ? '高自尊' : '低自尊'; break;
          case 'emotion': code = score > 0 ? 'O' : 'R'; name = score > 0 ? '开放' : '压抑'; break;
          case 'risk': code = score > 0 ? 'D' : 'C'; name = score > 0 ? '冒险' : '保守'; break;
          case 'control': code = score > 0 ? 'A' : 'G'; name = score > 0 ? '掌控' : '放松'; break;
          case 'independence': code = score > 0 ? 'U' : 'V'; name = score > 0 ? '独立' : '依赖'; break;
          case 'outlook': code = score > 0 ? 'P' : 'M'; name = score > 0 ? '乐观' : '悲观'; break;
          case 'reality': code = score > 0 ? 'I' : 'B'; name = score > 0 ? '理想' : '务实'; break;
          case 'competition': code = score > 0 ? 'W' : 'K'; name = score > 0 ? '竞争' : '合作'; break;
          case 'attachment': code = score > 0 ? 'X' : 'S'; name = score > 0 ? '亲密' : '疏离'; break;
          case 'stability': code = score > 0 ? 'Z' : 'Y'; name = score > 0 ? '稳定' : '波动'; break;
          case 'creativity': code = score > 0 ? 'N' : 'Q'; name = score > 0 ? '创新' : '传统'; break;
          default: code = '?'; name = '未知';
        }
  ...
  const generateRadarPath = () => {
    ...
    const radius = 50;
    const count = 15;
    const dimKeys = Object.keys(dimensions).filter(k => dimensions[k] && typeof dimensions[k].score === 'number');
      ...
      <div className="profile-card">
          ...
          <div className="profile-visual">
            <div className="radar-container">
              <svg viewBox="0 0 140 140" className="radar-chart">
                ...
                {[0.25, 0.5, 0.75, 1].map((scale, i) => (
                  <polygon
                    key={i}
                    points={Array(15).fill(0).map((_, j) => {
            ...
            <div className="dimension-pills">
              {topDimensions.slice(0, 3).map((dim, index) => (
                <div key={dim.key} className="dim-pill" style={{ animationDelay: `${index * 0.1}s` }}>
        ...
        {roast ? (
          ...
          <div className="profile-roast loading">
            ...
            <p className="roast-content">稍等，毒舌评论生成中...</p>
          </div>
        )}
        ...
        <div className="profile-footer">
          ...
          <div className="profile-brand">SoulBTI · 找到属于你的精准人格画像</div>
        </div>
      </div>
    </div>
  );
